...
module.exports = grammar({
  ...
  rules: {
    ...
    type_parameter: $ => choice(
      $.identifier,
      seq($.identifier, ':', $.type_constraints),
    ),
    ...
    function_positional_parameter_qualified_type_identifier: $ => choice(
      alias($.function_positional_parameter_identifier, $.identifier),
      seq($.package_identifier, $.dot_identifier),
    ),

    function_positional_parameter_option_type: $ => prec(TYPE_PREC.option, seq(alias($.function_positional_parameter, $.type), '?')),

    function_positional_parameter: $ => choice(
      alias($.function_positional_parameter_qualified_type_identifier, $.qualified_type_identifier),
      $.tuple_type,
      $.function_type,
      $.apply_type,
    ...
    function_labeled_parameter: $ => seq(
      $.parameter_label,
      $.type_annotation,
      optional($.default_value),
    ),

    function_optional_parameter: $ => seq(
    ...
    function_alias_declaration: $ => seq(
      optional($.attributes),
      optional($.visibility),
      'fnalias',
    ...
    constructor_payload: $ => seq(
      '(',
      list1(',', $.constructor_parameter),
      ')',
    ),

    constructor_tag_value: $ => seq(
      '=',
      $.integer_literal,
    ),
    ...
    trait_method_labeled_parameter: $ => seq(
      $.lowercase_identifier,
      '~',
      $.type_annotation,
    ),
    ...
    super_trait_declaration: $ => seq(
      ':',
      list('+', $.qualified_type_identifier),
    ),
